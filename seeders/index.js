...
async function seedUser () {
  try {
    ...
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Nella77' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'xntGuEbiOmMzHWL',
      'username':'Rosalia_Weber',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Rosalia_Weber' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
